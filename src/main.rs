...
/// Простая реализация эмбеддингов (для демонстрации)
/// В реальном проекте здесь был бы вызов Ollama API
...
async fn main() -> Result<()> {
    ...
    info!("Запуск Enhanced RAG Article Generator");
    
    // Создание генератора
    let mut generator = EnhancedRAGArticleGenerator::new(
        "http://127.0.0.1:8080".to_string(), // SearXNG host
        "qwen3:30b".to_string(),             // LLM model
        "nomic-embed-text:latest".to_string(), // Embedding model
        Some("http://localhost:11434".to_string()), // Ollama host
    );
    
    // Запрос для генерации статьи
    let article_query = "Write a comprehensive article about integrating LangChain, Ollama, and SearXNG for building RAG applications with local LLMs. Include technical implementation details, advantages, challenges, and real-world applications.";
    
    // Генерация статьи
    match generator.generate_article(article_query, 15).await {
        Ok(article) => {
            println!("\n{}", "=".repeat(80));
            ...
            // Сохранение в файл
            tokio::fs::write("enhanced_article_rust.md", &article).await?;
            info!("Статья сохранена в файл: enhanced_article_rust.md");
        }
        Err(e) => {
